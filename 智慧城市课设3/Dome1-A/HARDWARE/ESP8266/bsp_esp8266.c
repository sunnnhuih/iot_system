...
char  Esp8266_rx_buf [ RX_BUF_MAX_LEN ];
__IO u16 Esp8266_rx_flag;
__IO u16 Esp8266_rx_length;
...
/**
  * @brief  配置 ESP8266 USART 的 中断服务函数
  * @param  无
  * @retval 无
  */
...
{	
	...
	if ( USART_GetITStatus ( macESP8266_USARTx, USART_IT_RXNE ) != RESET )
	{
		...
		if ( Esp8266_rx_length < ( RX_BUF_MAX_LEN - 1 ) )                       //预留1个字节写结束符
			Esp8266_rx_buf[Esp8266_rx_length++] = ucCh;
	}
	if ( USART_GetITStatus( macESP8266_USARTx, USART_IT_IDLE ) == SET )                                         //数据帧接收完毕
	{
    Esp8266_rx_flag = 1;
		
		Esp8266_rx_buf [ Esp8266_rx_length ]  = '\0';
    printf("ucCh =%s\n",Esp8266_rx_buf);
		
		strx=strstr((const char*)Esp8266_rx_buf,(const char*)"LED");//返回OK
		if(strx!=NULL)
		{
			printf("\r\n\r\n\r\n%d\r\n\r\n",*(strx+4));
			switch(*(strx+3))
			{
				case '1':switch(*(strx+4))
								{
									case '0': ;break;
									case '1': ;break;
									default:break;
								}break;
				case '2':switch(*(strx+4))
								{
									case '0': ;break;
									case '1': ;break;
									default:break;
								}break;
				case '3':switch(*(strx+4))
								{
									case '0': ;break;
									case '1': ;break;
									default:break;
...
void Usart3_SendArr(char *Data,uint16_t Len)
{ 
	uint16_t i;
	for(i=0; i<Len; i++)
	{
...
{    
	Esp8266_rx_length = 0;               //从新开始接收新的数据包
...
//	macPC_Usart ("%s",Esp8266_rx_buf);
  
	if ( ( reply1 != 0 ) && ( reply2 != 0 ) )
		return ( ( bool ) strstr (Esp8266_rx_buf, reply1 ) || 
						 ( bool ) strstr (Esp8266_rx_buf, reply2 ) ); 
 	
	else if ( reply1 != 0 )
		return ( ( bool ) strstr (Esp8266_rx_buf, reply1 ) );
	
	else
		return ( ( bool ) strstr (Esp8266_rx_buf, reply2 ) );
...
uint8_t ESP8266_Get_LinkStatus ( void )
{
	...
	{
		if ( strstr (Esp8266_rx_buf, "STATUS:2\r\n" ) )
			return 2;
		
		else if ( strstr ( Esp8266_rx_buf, "STATUS:3\r\n" ) )
			return 3;
		
		else if ( strstr ( Esp8266_rx_buf, "STATUS:4\r\n" ) )
			return 4;		
...
uint8_t ESP8266_Get_IdLinkStatus ( void )
{
	...
	if ( ESP8266_Cmd ( "AT+CIPSTATUS", "OK", 0, 500 ) )
	{
		if ( strstr ( Esp8266_rx_buf, "+CIPSTATUS:0," ) )
			ucIdLinkStatus |= 0x01;
		else 
			ucIdLinkStatus &= ~ 0x01;
		
		if ( strstr ( Esp8266_rx_buf, "+CIPSTATUS:1," ) )
			ucIdLinkStatus |= 0x02;
		else 
			ucIdLinkStatus &= ~ 0x02;
		
		if ( strstr ( Esp8266_rx_buf, "+CIPSTATUS:2," ) )
			ucIdLinkStatus |= 0x04;
		else 
			ucIdLinkStatus &= ~ 0x04;
		
		if ( strstr ( Esp8266_rx_buf, "+CIPSTATUS:3," ) )
			ucIdLinkStatus |= 0x08;
		else 
			ucIdLinkStatus &= ~ 0x08;
		
		if ( strstr ( Esp8266_rx_buf, "+CIPSTATUS:4," ) )
			ucIdLinkStatus |= 0x10;
		else 
			ucIdLinkStatus &= ~ 0x10;	
...
uint8_t ESP8266_Inquire_ApIp ( char * pApIp, uint8_t ucArrayLength )
{
  ...
  ESP8266_Cmd ( "AT+CIFSR", "OK", 0, 500 );
	...
	pCh = strstr ( Esp8266_rx_buf, "APIP,\"" );
...
uint8_t ESP8266_Inquire_StaIp ( char * pApIp, uint8_t ucArrayLength )
{
  ...
  ESP8266_Cmd ( "AT+CIFSR", "OK", 0, 500 );
	...
	pCh = strstr ( Esp8266_rx_buf, "STAIP,\"" );
...
char * ESP8266_ReceiveString ( FunctionalState enumEnUnvarnishTx )
{
	...
	while ( ! Esp8266_rx_flag );
	Esp8266_rx_buf [Esp8266_rx_length ] = '\0';
	
	if ( enumEnUnvarnishTx )
		pRecStr = Esp8266_rx_buf;
	
	else 
	{
		if ( strstr ( Esp8266_rx_buf, "+IPD" ) )
			pRecStr = Esp8266_rx_buf;
...
uint8_t ESP8266_CWLIF ( char * pStaIp )
{
  ...
  ESP8266_Cmd ( "AT+CWLIF", "OK", 0, 100 );
	...
	pCh = strstr ( Esp8266_rx_buf, "," );
	
	if ( pCh )
	{
		pCh1 = strstr ( Esp8266_rx_buf, "AT+CWLIF\r\r\n" ) + 11;
	  ucLen = pCh - pCh1;
	}
